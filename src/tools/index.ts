...
export function registerCompanyTools(server: McpServer, client: TwentyClient) {
  // Operator-defined custom fields for the Company object (empty by default).
  // See src/config/custom-fields.ts and .env.example (CUSTOM_COMPANY_FIELDS).
  const companyCustomFields = loadCustomFields('company');
  ...
  server.tool(
    ...
    'Update an existing company in Twenty CRM',
    {
      ...
      ...customFieldsZodShape(companyCustomFields),
    },
    async (args) => {
    try {
      ...
      const updates = {
        ...
        ...pickCustomFieldValues(companyCustomFields, updateData as Record<string, unknown>),
      };
...
export function registerTaskTools(server: McpServer, client: TwentyClient) {
  server.tool(
    ...
    'Create a new task in Twenty CRM',
    {
      ...
      body: z.string().optional().describe('Task description (plain text — markdown formatting is preserved in markdown field but rendered as literal text in the rich-text view)'),
    ...
    try {
      ...
      const task = await client.createTask({ ...rest, bodyV2: serializeRichText(body) });
      return {
        content: [{
          type: 'text' as const,
  ...
  server.tool(
    ...
    'Create a new note in Twenty CRM',
    {
      ...
      body: z.string().describe('Note content (plain text — markdown formatting is preserved in markdown field but rendered as literal text in the rich-text view)'),
    },
    async (args) => {
    try {
      const noteData = { title: args.title, bodyV2: serializeRichText(args.body) };
  ...
  server.tool(
    ...
    async (_args) => {
      try {
        ...
        // BUGFIX (#12): surface a structured error to the caller instead of
        // letting findOrphanedRecords swallow failures and return empty arrays
        // (which fabricated a false "no orphaned records" result). isError marks
        // this as a tool failure, matching the pattern used elsewhere (e.g.
