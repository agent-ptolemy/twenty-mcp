...
// Twenty's bodyV2 stores plain text only — markdown formatting in the input is
// preserved verbatim in `markdown` and rendered as literal text in `blocknote`.
export function serializeRichText(plain: string | undefined): RichTextInput | undefined {
...
export class TwentyClient {
  ...
  async listAllObjects(options: MetadataQueryOptions = {}): Promise<ObjectSummary> {
    // BUGFIX (#14): the previous query requested `objects` with no `paging`/`first`
    // argument. Twenty's metadata GraphQL API (`/metadata`) returns a Relay
    // connection that is page-limited by default, so a workspace with many
    // standard + system objects only surfaced the first page (observed: just
    // Company / Task / Opportunity, with `person` and `note` missing even though
    // the person-based data tools worked against the same workspace).
    //
    // Fix: page through the connection explicitly via `first` + `after` until
    // `pageInfo.hasNextPage` is false, so we always collect EVERY object
    // regardless of the server's default page size or object ordering. We use a
    // large `first` to minimise round-trips; the cursor loop is the actual
    // correctness guarantee.
    const query = `
      query GetObjectMetadata($first: Int, $after: ConnectionCursor) {
        objects(paging: { first: $first, after: $after }) {
          edges {
            node {
              id
              nameSingular
    ...
    // Hard cap on iterations as a safety net against an unexpected server that
    // never reports hasNextPage=false (avoids an infinite loop).
    for (let i = 0; i < 100; i++) {
      const result = await this.metadataClient.request(query, {
        first: PAGE_SIZE,
        after,
      }) as {
        objects: {
          edges: { node: ObjectMetadata; cursor: string }[];
          pageInfo: { hasNextPage: boolean; endCursor: string | null };
        };
      };

      for (const edge of result.objects.edges) {
        allObjects.push(edge.node);
      }
  ...
  async findOrphanedRecords(): Promise<OrphanedRecords> {
    ...
    // BUGFIX (#12): this method previously wrapped its entire body in a
    // try/catch that only logged via console.warn and then returned the empty
    // `orphaned` skeleton. On any GraphQL error that meant the tool reported
    // "no orphaned records" — fabricated, false-clean results — instead of
    // surfacing the failure. We now let errors propagate so the tool layer can
    // return a structured isError response. The only thing we defend against
    // here is null relation connections on individual records (see below).
    //
    // Twenty returns the relation connection (e.g. `people`, `opportunities`)
    // as null for some records rather than `{ totalCount: 0 }`. We treat a
    // missing/null connection as a count of 0 via `?.totalCount ?? 0`. We
    // deliberately do NOT skip such records: a null connection means "no linked
    // records of that type", which is exactly the orphaned condition we are
    // hunting for, so skipping would hide real orphans (the more dangerous
    // failure mode). Counting null as 0 keeps the orphan detection inclusive
    // and only ever understates a non-orphan's secondary counts, never
    // fabricates a clean result.
    ...
    orphaned.companies = companiesResult.companies.edges
      ...
      .filter((company: any) => (company.people?.totalCount ?? 0) === 0)
      .map((company: any) => ({
        id: company.id,
        name: company.name,
        contactCount: 0,
        opportunityCount: company.opportunities?.totalCount ?? 0
      }));
