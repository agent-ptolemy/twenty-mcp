...
**Optional tuning:**
- `SESSION_TIMEOUT_MS` — idle session timeout in milliseconds. Default `1800000` (30 minutes). Eviction is recoverable: requests carrying an evicted (or otherwise unknown) session ID are transparently resurrected in place, so gateways that never re-initialise on `-32000 Session not found` keep working across idle periods and server restarts. The timeout now mainly bounds idle session memory; the default is fine for most setups.
...
The server ships with a `Dockerfile` and `docker-compose.yml`, so the simplest way to run it in production is Docker:
...
Provide configuration via a `.env` file (see [`.env.example`](.env.example)) — at minimum a `TWENTY_API_KEY` and `TWENTY_BASE_URL`. Put the container behind a reverse proxy (Caddy, nginx, Traefik) for TLS and host it wherever you run containers.

For multi-user OAuth deployments, see [OAUTH.md](OAUTH.md). For IP allowlisting in front of the server, see [docs/IP_PROTECTION.md](docs/IP_PROTECTION.md).
...
### Configurable custom fields on companies

Twenty lets each workspace define its own custom fields on the Company object. Because those fields only exist on your instance, they are **configured at runtime** rather than hardcoded. When you declare custom fields, `update_company` automatically accepts them as optional inputs and writes them through to Twenty.

Declare them with the `CUSTOM_COMPANY_FIELDS` environment variable (a JSON array), or via a `CUSTOM_FIELDS_FILE` pointing at a JSON file:

```bash
CUSTOM_COMPANY_FIELDS='[
  {"name":"renewalDate","type":"string","description":"Renewal date (ISO format)"},
  {"name":"seatCount","type":"number","description":"Licensed seat count"},
  {"name":"isStrategic","type":"boolean","description":"Strategic account flag"}
]'
```

Each entry takes `name`, `type` (`string` | `number` | `boolean`), and an optional `description`. The field name must match the API name of the custom field in your Twenty workspace. With no configuration set, the server exposes only standard Twenty fields. See [`.env.example`](.env.example) and [`src/config/custom-fields.ts`](src/config/custom-fields.ts).
...
| `create_note` | `twenty-client.ts` | `body` → `bodyV2 { blocknote, markdown }`, removed `authorId` |
| `create_task` | `twenty-client.ts` | `body` → `bodyV2 { blocknote, markdown }` |
| `get_tasks` | `twenty-client.ts` | `body` → `bodyV2 { blocknote, markdown }` |
| `getActivities` | `twenty-client.ts` | `body` → `bodyV2 { blocknote, markdown }` on both Task and Note reads |
| `create_comment` | `activities.ts` | Removed — Twenty deleted the `Comment` entity from the API |
| Metadata tools | `twenty-client.ts` | Split `/metadata` endpoint, `ObjectFilterInput` → `ObjectFilter` |

See [`docs/RICH_TEXT.md`](docs/RICH_TEXT.md) for the `bodyV2` payload contract, BlockNote document shape, and plain-text input semantics.
